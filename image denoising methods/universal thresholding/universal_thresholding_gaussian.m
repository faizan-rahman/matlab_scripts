function universal_thresholding_gaussian(input, variance, decomposition_level)
...
noisy_image = imnoise(input_image,'gaussian', 0, variance);
figure; 
imshow(noisy_image);
title('noisy image');
...
universal_threshold = sigma*sqrt(2*log(M));

hard_coefficients = [C(1:S(1,1)^2), hthresh(C(S(1,1)^2+1:length(C)), universal_threshold)]; %apply hard thresholding on wavelet matrix
...
soft_coefficients = [C(1:S(1,1)^2), sthresh(C(S(1,1)^2+1:length(C)), universal_threshold)]; %apply hard thresholding on wavelet matrix
